@extends('layouts.app')
@section('title','Unposting Jurnal Kas Keluar')

@section('breadcrumb')
<div class="app-content-header">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6"><h5 class="mb-2">Unposting Jurnal Kas Keluar</h5></div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('jurnal.kaskeluar') }}">Jurnal Kas Keluar</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Unposting Jurnal Kas Keluar</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card card-success card-outline mb-4">
                <div class="card-header"><div class="card-title">Unposting Jurnal Kas Keluar</div></div>
                
                <form action="javascript:void(0)" enctype="multipart/form-data" id="form_data">
                    @csrf
                    @method("post")
                    <input type="hidden" name='jenis' value="JKK">
                    ...
                    <div class="card-footer">
                        ...
                        <button type="submit" id="btn-submit" class="btn btn-success btn-flat btn-sm float-end">
                            <i class="fa fa-bolt"></i> Unposting
                        </button>
                    </div>
                </form>
            </div>  
  ...
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content border-0 shadow-lg">
      <div class="modal-header bg-success text-white py-2">
        <h6 class="modal-title" id="progressModalLabel"><i class="fa fa-spinner"></i> Proses Data</h6>
...
function load_data(){
    $('#t_data').DataTable({
        ...
        responsive: true,
        ajax: {
            url: "{{ route('jurnal.kaskeluar.unposting') }}",
            data: function (d) {
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            if(result.status === "ready"){
                ...
                modalProgress.hide();
                Swal.fire('Tidak Ada Data', 'Tidak ditemukan jurnal untuk diunposting.', 'info');
                $("#btn-submit").html("<i class='fa fa-bolt'></i> Posting");
                $("#btn-submit").prop("disabled", false);
            }
        },
        error: function(e){
            $("#btn-submit").html("<i class='fa fa-bolt'></i> Unosting");
            $("#btn-submit").prop("disabled", false);
...
function startBatchPosting(totalBatch, modalProgress) {
    ...
    function nextBatch(){
        $.ajax({
            type: "POST",
            url: "{{ route('jurnal.unposting_batch') }}",
