...
DROP PROCEDURE IF EXISTS sp_jurnal_mgr_jkm$$

CREATE PROCEDURE sp_jurnal_mgr_jkm(
    IN p_tanggal DATE,
...
BEGIN
    ...
    -- PREFIX TETAP (MIGRASI)
    SET v_prefix  = 'MGR';
    SET v_periode = DATE_FORMAT(p_tanggal, '%Y%m');

    -- Ambil nomor lanjutan yang sama
    SELECT IFNULL(
