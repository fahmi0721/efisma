...
class JurnalUangMukaProcess
{
    ...
    public static function index($request){
        ...
        try {
             ...
             */
            $detailGroup = self::cleansingJurnalDetail($spreadsheet);
            // throw new Exception(json_encode($detailGroup), 500);


            /**
             * Proses cleansing dan insert jurnal header
             */
            self::prosesJurnalHeaderDanDetail($spreadsheet, $request, $detailGroup);
    ...
    private static function prosesJurnalHeaderDanDetail($spreadsheet, $request, array $detailGroup)
    {
        ...
        for ($row = 3; $row <= $highestRow; $row++) {
            ...
            foreach ($details as $detail) {
                JurnalDetail::create([
                    ...
                    'cabang_id' => $cabangId !== '' ? $cabangId : null,
                    'jurnal_id_secound' => empty($detail['jurnal_id_secound']) ? $jurnalHeader->id : $detail['jurnal_id_secound'],
    ...
    private static function cleansingJurnalDetail($spreadsheet): array
    {
        ...
        for ($row = 4; $row <= $highestRow; $row++) {
            ...
            $deskripsi         = trim((string) $sheet->getCell('D' . $row)->getCalculatedValue());
            ...
            $UangMukaID = $sheet->getCell('G' . $row)->getCalculatedValue(); // Uang Muka ID / nilai G
            ...
            if (
                $templateHeaderId === '' &&
                $akunId === '' &&
                ($deskripsi === '' || $deskripsi === 'PJ Uang Muka #N/A') &&
