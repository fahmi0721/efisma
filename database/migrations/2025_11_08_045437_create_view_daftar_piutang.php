    ...
    public function up(): void
    {
        DB::statement("
            ...
            SELECT
                ...
                p.is_vendor,
                p.is_customer,
                COALESCE(dp.total_piutang, 0) AS total_tagihan,
                COALESCE(SUM(CASE WHEN jk.status = 'posted' THEN pp.jumlah ELSE 0 END), 0) AS total_pelunasan,
                COALESCE(dp.total_piutang, 0) -
                COALESCE(SUM(CASE WHEN jk.status = 'posted' THEN pp.jumlah ELSE 0 END), 0) AS sisa_piutang,
                TO_DAYS(CURDATE()) - TO_DAYS(j.tanggal) AS umur_piutang
            FROM jurnal_header j
            JOIN (
                SELECT
                    d.jurnal_id,
                    SUM(d.debit) AS total_piutang
                FROM jurnal_detail d
                JOIN m_akun_gl a2 ON a2.id = d.akun_id
                WHERE a2.kategori = 'piutang'
                GROUP BY d.jurnal_id
            ) dp ON dp.jurnal_id = j.id
            ...
            WHERE
                j.status = 'posted'
                AND COALESCE(dp.total_piutang, 0) > 0
                AND NOT EXISTS (
                    SELECT 1
